...
@cache
def common(springs: str, broken: str) -> int:
    if len(broken) == 0:
        if "#" not in springs:
            return 1
        else:
    ...
    for start_idx in range(len(springs)):
        if (
            ...
            if start_idx + seq_len + 1 < len(springs):
                ...
                assignments.append("")
    return sum([common(ass, broken) for ass in assignments])


def part_a(data: str):
    rows = parse(data)
    return sum([common(springs, broken) for springs, broken in rows])
